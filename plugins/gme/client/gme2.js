...
GraphiticalModelEditor.macroOperation = {
		onNodeAdded : 	{
			name : "onNodeAdded"
		},
		onChildAdded : {
...
function GraphiticalModelEditor(_key, _name, _diagram, _dinfo, _editordef, _modelController, _ctool, _editor, option) {
	...
	function removeNodeFromModel(selectedNode) {
		var result = modelController.macroOperation(macroOperation.onNodeRemovedFromModel.name,
				editor_key,
				function(mc){
	...
	function removeConnectionFromModel(selectedConnection) {
		var result = modelController.macroOperation(macroOperation.onConnectionRemovedFromModel.name,
				editor_key,
				function(mc){
		    ...
		    items: [{
		        ...
		        handler: function(){
					...
					if(ninfo) {
						...
						modelController.updateProperties(addedNodeInstance,
								{x:Math.floor(x),
								 y:Math.floor(y),
								 w:80,
								 h:40,
								 uri:modelController.getURI(elem)},
								editor_key);
						...
						if(start && end) {
							...
							modelController.updateProperties(addedConnectionInstance,
									{uri:modelController.getURI(elem)},
									editor_key);
	...
	return {
		...
		Initialize : function() {
			...
			editor = new clooca.diagrameditor.DiagramEditor(editor_key,
					svg,
					editfield,
					diagram,
					pkg,
			...
			*/
			...
			editor.on("onMoved", function(e) {
				var result = modelController.macroOperation(macroOperation.onNodeMoved.name,
						editor_key,
						function(mc){
			...
			editor.on("onMovePoint", function(e) {
				var result = modelController.macroOperation(macroOperation.onPointMoved.name,
						editor_key,
						function(mc){
							var nodeInstance = modelController.get(e.target.getNodeURI());
							var changes = {};
			...
			editor.on("onResized", function(e) {
				var result = modelController.macroOperation(macroOperation.onNodeResized.name,
						editor_key,
						function(mc){
							var nodeInstance = mc.get(e.target.getNodeURI());
							mc.updateProperties(nodeInstance,
									{w:Math.floor(e.w),h:Math.floor(e.h)},
									editor_key);
							return {
								id:e.target.getID(),
								nodeURI:e.target.getNodeURI()
			...
			editor.setAddNodeListenner(function(e) {
				var result = modelController.macroOperation(macroOperation.onNodeAdded.name,
						editor_key,
						function(mc){
							...
							mc.updateProperties(addedNodeInstance,
									{x:(e.x|0),
									 y:(e.y|0),
									 w:e.w,
									 h:e.h,
									 uri:modelController.getURI(addedInstance)},
									editor_key);
							return {
			...
			editor.setAddChildListenner(function(e) {
				...
				var asso = ctool.getAssociation(ninfo.containFeature);
				var result = modelController.macroOperation(macroOperation.onChildAdded.name,
						editor_key,
						function(mc){
			...
			editor.setAddConnectionListenner(function(e) {
				...
				var result = modelController.macroOperation(macroOperation.onConnectionAdded.name,
						editor_key,
						function(mc){
							...
							if(ctool._can_Contain(cinfo.containFeature, start._sys_meta)) {
								added_instance = modelController.addInstance(start,
										containAssociation,
										e.metaElement,
										editor_key);
							}else if(ctool._can_Contain(cinfo.containFeature, diagram._sys_meta)) {
								added_instance = modelController.addInstance(diagram,
										containAssociation,
										e.metaElement,
										editor_key);
							}
							...
							var connectionClass = ctool.getClass("Editor.DiagramEditor.Connection");
							var addedConnectionInstance = modelController.addInstanceRaw(diagram,
									metadiagram.associations['_connections'],
									connectionClass,
									editor_key);
		...
		//idはノード、コネクションのID
		selectByOther : function(id) {
			var node = editor.getElement(id);
		},
		//idはノード、コネクションのID
		deselectByOther : function(id) {
			var node = editor.getElement(id);
		},
		//idはノードのID
		moveByOther : function(id, nodeElem) {
			var node = editor.getElement(id);
			console.log(node);
			if(node) {
		...
		//idはノードのID
		resizeByOther : function(id, nodeElem) {
			var node = editor.getElement(id);
			if(node) {
				node.setSize(nodeElem.w, nodeElem.h);
			}
		},
		addNodeByOther : function(instance, nodeInstance, metaNodeName) {
			for(var key in dinfo.nodes) {
		...
		addChildNodeByOther : function(instance, nodeInstance, parentInstance, parentNodeURI, parentMetaNodeName, metaNodeName) {
			for(var key in dinfo.nodes) {
		...
		addConnectionByOther : function(instance, nodeInstance, startURI, endURI, metaConnectionName) {
		...
		execute_onNodeAdded : function(opResult) {
		...
		executeOperation : function(op, sender) {
			this.executeOperationResult(op, sender);
		},
		/*
		 * opResultはオペレーション結果です。他の場所でオペレーションが実行され、その結果をもとに処理をします。
		 */
		executeOperationResult : function(opResult, sender) {
			console.log("executeOperationResult", opResult.eventName);
			if(opResult.eventName===macroOperation.onNodeAdded.name) {
		   		this.addNodeByOther(
		   				modelController.get(opResult.eventObject.instanceURI),
		   				modelController.get(opResult.eventObject.nodeURI),
		   				opResult.eventObject.metaNode);
			}else if(opResult.eventName===macroOperation.onChildAdded.name){
				this.addChildNodeByOther(modelController.get(opResult.eventObject.instanceURI),
		   				modelController.get(opResult.eventObject.nodeURI),
		   				modelController.get(opResult.eventObject.parentURI),
		   				opResult.eventObject.parentNodeURI,
		   				opResult.eventObject.parentMetaNode,
		   				opResult.eventObject.metaNode);
			}else if(opResult.eventName===macroOperation.onConnectionAdded.name){
				this.addConnectionByOther(modelController.get(opResult.eventObject.instanceURI),
		   				modelController.get(opResult.eventObject.connectionURI),
		   				opResult.eventObject.startURI,
		   				opResult.eventObject.endURI,
		   				opResult.eventObject.metaConnection);
			}else if(opResult.eventName===macroOperation.onSelected.name) {
			}else if(opResult.eventName===macroOperation.onNodeMoved.name) {
				this.moveByOther(
						opResult.eventObject.nodeURI,
						modelController.get(opResult.eventObject.nodeURI));
		...
		executeOperationResultInverse : function(opResult, sender) {
			...
			if(opResult.eventName===macroOperation.onNodeAdded.name) {
				...
				this.movePointByOther(opResult.eventObject.nodeURI);
		   		...
		   		if(opResult.eventObject.parentURI) {
					this.addChildNodeByOther(
							modelController.get(opResult.eventObject.id),
			   				modelController.get(opResult.eventObject.nodeURI),
			   				modelController.get(opResult.eventObject.parentURI),
			   				opResult.eventObject.parentNodeURI,
			   				opResult.eventObject.parentMetaNode,
				...
				this.addConnectionByOther(modelController.get(opResult.eventObject.id),
		   				modelController.get(opResult.eventObject.connectionURI),
		   				opResult.eventObject.startURI,
		   				opResult.eventObject.endURI,
		   				opResult.eventObject.metaConnection);
		...
		highlight : function(uri, col) {
			var node = editor.getElementByURI(uri);
			node.highlight(col);
		},
		highlightById : function(attr,id, col) {
